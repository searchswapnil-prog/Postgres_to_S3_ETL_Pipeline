...
if known_args.WORKFLOW_NAME and known_args.WORKFLOW_RUN_ID:
    ...
    etl_run_ts = props.get('etl_run_ts')
else:
    args_fallback = getResolvedOptions(sys.argv, ["etl_run_ts"])
    etl_run_ts = args_fallback["etl_run_ts"]
...
if known_args.WORKFLOW_NAME and known_args.WORKFLOW_RUN_ID:
    response = glue_client.get_workflow_run_properties(Name=known_args.WORKFLOW_NAME, RunId=known_args.WORKFLOW_RUN_ID)
    props = response.get('RunProperties', {})
    props['dq_run_ts'] = run_ts
    props['dq_status'] = dq_status
    ...
    logger.info(f"Workflow properties updated with dq_run_ts={run_ts}, dq_status={dq_status}")
