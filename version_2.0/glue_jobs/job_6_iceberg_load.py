...
if known_args.WORKFLOW_NAME and known_args.WORKFLOW_RUN_ID:
    ...
    dq_run_ts = props.get('dq_run_ts')
    dq_status = props.get('dq_status')
else:
    args_fallback = getResolvedOptions(sys.argv, ["dq_run_ts", "dq_status"])
    dq_run_ts = args_fallback["dq_run_ts"]
    dq_status = args_fallback["dq_status"]
